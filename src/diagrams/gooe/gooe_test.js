gpub.diagrams.gooe.gooeTest = function() {
  ...
  var gooeDiagramArray = gpub.diagrams.gooe.diagramArray;

  test("Char mapping is sensible", function() {
    var symb = glift.bridge.flattener.symbols;
    for (var key in gpub.diagrams.gooe.charMapping) {
  ...
  test("Test Gooe string generation", function() {
    ...
    var f = flatten(movetree, goban);
    var gooeArray = gooeDiagramArray(f);
    var strOut = gpub.diagrams.gooe.diagramArrToString(gooeArray);
    ok(strOut !== undefined);
  });
};
